...
`*` Переписано окно всплывающего сообщения для лучшего отображения длинных сообщений (также добавлены настройки <em>extensions.handyclicks.notifyMessageMaxWidth</em> и <em>extensions.handyclicks.notifyMessageMaxHeight</em>), также теперь используются CSS transitions для визуализации таймера закрытия.<br>
...
`*` Добавлена возможность не открывать одно и то же всплывающее сообщение дважды (настройка <em>extensions.handyclicks.notifyDontOpenTwice</em>).<br>
