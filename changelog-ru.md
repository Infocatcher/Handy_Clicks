...
`x` Исправлено переименование выгруженных вкладок.<br>
`x` Исправлен импорт устаревшего js-формата (не JSON): исправлено конвертирование свойства $all.<br>
`+` Добавлена базовая поддержка локализованных названий из версии 0.2.0b1.<br>
`x` Исправлен поиск в дереве настроек после удаления отложенного действия: добавлено удаление ссылки на удаленный DOM-узел отложенного действия.<br>
`x` Исправлено асинхронное чтение файлов в функции `handyClicksUtils.readFromFileAsync()`: добавлена обработка ошибок наподобие out of memory из функции `TextDecoder.decode()`.<br>
`+` Добавлена поддержка псевдонима %hc_ScriptsDir% для настроек из версии 0.2.0b1.<br>
