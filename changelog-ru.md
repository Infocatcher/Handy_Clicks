...
`+` Добавлен экспорт/импорт для отдельных *.js файлов (`//> %hc_ScriptsDir%/example.js` в коде, также добавлен %hc_ScriptsDir% для %profile%/handyclicks/scripts/ и поисковая замена %data%, настройка <em>extensions.handyclicks.sets.importPaths</em> для разрешения импорта/экспорта: "D:\hcScripts|%CurProcD%\hcScripts") (<a href="https://github.com/Infocatcher/Handy_Clicks/issues/32">#32</a>).<br>
...
`+` Добавлено меню для вставки поисковых замен, например, %custom% (кликните средней кнопкой мыши или нажмите любой модификатор, чтобы оставить только одну поисковую замену).<br>
...
`x` Исправлена работа Ctrl+Down/Up в текстовых полях редактора в Firefox 52+ (аналог навигации с помощью Tab/Ctrl+Tab).<br>
...
`*` Выделить всё найденное: прокручивается до ближайшей найденной строки, если ни один из результатов поиска не был виден.<br>
...
`+` Редактор: добавлена кнопка-меню и контекстное меню для открытия сохраненных сочетаний кнопок мыши и модификаторов.<br>
...
`+` Редактор: добавлена кнопка и контекстное меню для переименования сочетания кнопки мыши и модификаторов (сочетание клавиш Ctrl+Shift+R/F2).<br>
...
`+` Добавлена возможность сортировки дерева (кликом по столбцу или с помощью меню Вид – Сортировка), также добавлены настройки <em>extensions.handyclicks.sets.rememberSort</em>, <em>extensions.handyclicks.sets.treeSortAutoInlineDrawMode</em> и <em>extensions.handyclicks.sets.treeSortAutoCollapseDelayedAction</em>.<br>
...
`*` При автоматическом восстановлении настроек из резервной копии используется самый новый файл, найденный среди всех подходящих резервных копий (особенно полезно после удаления дубликатов резервных копий).<br>
`+` Редактор: добавлена команда для переключения на дерево настроек (F3).<br>
`+` Редактор: добавлено меню «Настройки».<br>
`+` Редактор: добавлены пункты меню для настроек <em>extensions.handyclicks.editor.ui.invertWindowTitle</em> и <em>extensions.handyclicks.editor.ui.compact</em>.<br>
`+` Редактор: добавлен пункт меню для открытия настроек.<br>
`+` Настройки, редактор: добавлен пункт меню для перезагрузки настроек (для уже существовавшего сочетания клавиш F5/Ctrl+R).<br>
`*` Добавлен запрос подтверждения перед перезагрузкой настроек (настройка <em>extensions.handyclicks.ui.confirmReload</em>).<br>
`x` Перезагрузка настроек добавленного типа: убран вывод некорректного предупреждения про несохраненные настройки.<br>
`x` Переключение с помощью прокрутки: исправлена прокрутка радио-пунктов внутри закрытых меню (отправляется поддельное событие «popupshowing» для инициализации).<br>
`*` Улучшен счетчик поиска/фильтрации: добавлено отображение общего количества настроек.<br>
`+` Добавлена поисковая замена %internal% для настроек, использующих встроенные функции.<br>
`*` Добавлен перевод фокуса на дерево настроек после выполнения относящихся к дереву команд.<br>
`*` После клика средней кнопкой мыши теперь открывается на редактирование только одна настройка (а не все выделенные).<br>
`+` Добавлен пункт меню «переключиться на дерево» (чтобы показать сочетание клавиш Ctrl+T).<br>
`*` Испорт из буфера обмена: добавлена поддержка файлов с настройками.<br>
`*` Добавлено меню для отключения/восстановления предупреждающих сообщений.<br>
`+` Встроенный редактор: добавлено сочетание клавиш Ctrl+/ для комментирования кода (как в Scratchpad).<br>
`x` Редактор: исправлено автодополнение для для слов с «-» (например, «background-color») и слов с цифрами (например, «Float32Array»).<br>
`*` Редактор: автодополнение для свойства «style».<br>
`*` Изменены ограничения для настроек редактора (<em>extensions.handyclicks.editor.autocompleteMinSymbols</em>: 30 -> 10, <em>extensions.handyclicks.editor.tabSize</em>: 100 -> 300, ограничение размера шрифта: 48 -> 64).<br>
`*` Улучшен счетчик поиска/фильтрации: клик левой/правой кнопкой мыши – найти далее вниз/вверх, клик средней кнопкой мыши – выделить всё найденное/снять всё выделение.<br>
`+` Редактор: F2 для открытия about:config для зависимой настройки, находящейся в фокусе.<br>
`+` Добавлена возможность открыть папку с временными файлами из предупреждения «По-видимому произошло падение браузера: как минимум один временный файл для внешнего редактора не был удалён».<br>
`x` Исправлено резервное копирование при тестировании и <em>extensions.handyclicks.sets.backupTestDepth</em> = 1.<br>
`+` Добавлена запрос на сохранение настроек перед выходом или перезапуском браузера.<br>
`*` Редактор: улучшено редактирование добавленных типов с одинаковыми названиями (тем не менее, рекомендуется задавать разные имена).<br>
`x` Исправлена реализация `nsIProtocolHandler.newURI()` для протокола handyclicks://… в Firefox 58+.<br>
`*` Улучшена производительность импорта: повторно используются данные, уже распарсенные на стадии проверки корректности.<br>
`*` Поведение элементов управления: добавлен принудительное открытие контекстного меню после клика правой кнопкой мыши с любым модификатором.<br>
`*` Внешний редактор: добавлена командная строка по умолчанию для Visual Studio Code, CudaText, SynWrite, Atom и обновлена для AkelPad (только Windows).<br>
`*` Настройки: улучшена обработка ошибок чтения/записи.<br>
`*` Открыть/закрыть настройки: теперь выводится запрос на сохранение изменений.<br>
`*` Выход из режима импорта: теперь выводятся запросы на сохранение изменений в открытых редакторах.<br>
`+` Настройки: добавлены пункты меню и сочетания клавиш для раздельной перезагрузки настроек кликов / настроек about:config.<br>
`*` Редактор: добавлено примечание для клавиши Meta (только Mac OS).<br>
`*` Редактор: улучшена обработка несохраненного добавленного типа.<br>
`x` Fixed error line numbers in Pale Moon 28.5+ (looks like Pale Moon bug, now always decremented by 1).<br>
`x` Исправлены номера строк с ошибками в Pale Moon 28.5+ (похоже на баг в Firefox, теперь из номера строки всегда вычитается 2).<br>
`*` Улучшена производительность сохранения настроек, оптимизировано переименование файлов резервных копий (примерно в 2,4 раза быстрее при настройках по умолчанию).<br>
`*` Изменен метод handyClicksPrefSvc.registerDestructor(): также добавляется причина DESTROY_WINDOW_UNLOAD при закрытии последнего окна (что более интуитивно).<br>
`*` Копировать в формате HTML: дополнительно добавляются text/html данные, например, для WYSIWYG редокторов.<br>
`*` Добавлено переоткрытие всплывающей подсказки в режиме редактирования, чтобы принудительно вывести её поверх других появившихся всплывающих подсказок.<br>
`+` Добавлены поисковые замены %+lng%/%-lng% для добавленных функций с переведенными/непереведенным названиями.<br>
...
`*` Реализован поиск по полному оригинальному коду, если отображаемая строка была обрезана (настройка <em>extensions.handyclicks.sets.codeLengthLimit.limitSearch</em>).<br>
`+` Добавлена поисковая замена %long% для обрезанных слишком длинных строк кода (настройки <em>extensions.handyclicks.sets.codeLengthLimit</em> и <em>extensions.handyclicks.sets.codeLengthLimit.preserveLines</em>).<br>
`x` Настройки: функции для недоступных расширений больше не помечаются как дефектные (будут сохранены корректно).<br>
`+` Настройки, редактор: добавлено отображение альтернативных сочетаний клавиш во всплывающих подсказках.<br>
`*` Настройки: добавлена прокрутка к текущему результату поиска при переключении режима фильтра.<br>
`+` Настройки: добавлено восстановление текущей позиции поиска (для настройки <em>extensions.handyclicks.sets.rememberSearchQuery</em> = true).<br>
`+` Настройки: добавлено сочетание клавиш F1 (если в фокусе панель импорта) и сочетание клавиш Ctrl+Shift+D (глобальное) для отображения/скрытия всплывающей подсказки с информацией о связанных файлов.<br>
`*` Редактор: уточнено подтверждение переименования и сохранения: теперь просто выводится два запроса.<br>
`*` Редактор: чекбокс «Зафиксирован» заменен на кнопку.<br>
`x` Редактор: исправлено переключение на строку с ошибкой определения контекстного меню добавленного типа.<br>
`x` Восстановлена обработка двойного клика средней и правой кнопкой мыши в Firefox 53+ (теперь используется слушатель события auxclick и настройка <em>extensions.handyclicks.dblclickDelay</em>).<br>
`+` Редактор: добавлено сочетание клавиш Shift+F3 для открытия меню настроек.<br>
`*` Редактор: изменено сочетание клавиш для открытия меню сохраненных сочетаний кнопки мыши и модификаторов: Shift+F3 → Shift+F2 (для соответствия сочетанию клавиш F2 для переименования сочетания кнопки мыши и модификаторов).<br>
`*` Настройки: реализован поисковый токенизатор для комплексных запросов, разделенных пробелами (Пробел = И, -Нечто = НЕ Нечто):<br>
словоБезПробелов /RegExp/i 'строка целиком' "Строка с Учетом Регистра" -слово -/%off%.*RegExp2/s<br>
`+` Настройки: Импорт - Восстановить из резервной копии: добавлены специальные иконки для резервных копий с данными связанных файлов.<br>
`*` Настройки, редактор: добавлена подсветка неизвестных типов событий, для будущей совместимости.<br>
`+` Настройки: добавлено отображение данных связанных файлов в дереве (может быть отключено из меню Вид, настройки <em>extensions.handyclicks.sets.treeExpandFileData</em> и <em>extensions.handyclicks.sets.treeExpandFileDataImport</em>).<br>
`*` Настройки: улучшен частичный экспорт из импорта: теперь используются данных из импортируемых файлов (а не из уже существующих реальных).<br>
`*` Автоматическое резервное копирование: теперь также сохраняются данные файлов (настройка <em>extensions.handyclicks.sets.backupAutoSaveFilesData</em>).<br>
`+` Добавлена возможность отключения вычисления хэш-сумм для тестирования или в целях увеличения производительности в случае огромных настроечных файлов (не рекомендуется к изменению, используйте на свой страх и риск, настройка <em>extensions.handyclicks.sets.dontCalcHashes</em>).<br>
`+` Настройки: добавлен пункт контекстного меню для закрытия всех открытых редакторов.<br>
`x` Настройки: исправлено обновление интерфейса после импорта или сброса настроек about:config.<br>
`*` Теперь разрешен любой код после строки с заголовком-ссылкой на отдельный *.js файл (`//> %hc_ScriptsDir%/example.js`), например, для кода загрузчика для старых версий.<br>
`*` Удалено использование не рекомендуемого для нового кода <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/substr">String.prototype.substr()</a>.<br>
`+` Добавлена настройка .checkOtherTypes для действий с добавленным типом.<br>
`x` Настройки: исправлено обновление кнопки «Применить» после конвертирования настроек в новый формат.<br>
`+` Настройки: добавлены кнопки для тестирования/предпросмотра всплывающих уведомлений.<br>
`*` Откорректированы и обновлены локализации.<br>
