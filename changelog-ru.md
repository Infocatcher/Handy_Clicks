...
`+` Добавлен экспорт/импорт для отдельных *.js файлов (`//> %hc_PrefsDir%/scripts/example.js` в коде, также добавлен %hc_ScriptsDir% для %profile%/handyclicks/scripts/) (<a href="https://github.com/Infocatcher/Handy_Clicks/issues/32">#32</a>).<br>
...
`*` Добавлена специальная подсветка для пользовательских файлов (`//> %hc_PrefsDir%/scripts/example.js` в коде).<br>
...
`*` Небольшие внутренние улучшения кода.<br>
