var handyClicksPrefSvc = {
	...
	importSrc: function(tar, src) {
		...
		if(src.hasOwnProperty("json") && !(pv.prefs in src)) {
			...
			src = src.json;
			tar.currentVersion = src.version || 0;
			tar.types = src.types;
			tar.prefs = src.prefs;
			if(tar == this)
				this.ut._log("Import prefs from new JSON format, version: " + tar.currentVersion);
			return;
		}
		for(var p in pv)
